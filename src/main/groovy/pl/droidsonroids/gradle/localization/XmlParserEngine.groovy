...
class XmlParserEngine {
    ...
    XmlParserEngine(ConfigExtension config, File resDir) {
        ...
        if (config.outputFileName != null) {
            mCloseableInput = Utils.wrapReader(new FileReader(config.outputFileName))
        } else {
            throw new IllegalStateException()
        }

        mParser = new XmlParser((InputStream) mCloseableInput, config.sheetName, config.useAllSheets)
    }
    ...
    void parseXml() {
        mCloseableInput.withCloseable {
            ...
            for (String sheetName : sheets.keySet()) {
                ...
                def header = new XmlSourceInfo(allCells[0], mConfig, mResDir, outputFileName)
                parseCells(header, allCells)
            }
        }
    }

    private parseCells(final XmlSourceInfo sourceInfo, String[][] cells) {
